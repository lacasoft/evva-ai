...
const envSchema = z.object({
  ...
  TELEGRAM_WEBHOOK_URL: z.string().url().optional().or(z.literal('')).transform(v => v || undefined),
  TELEGRAM_SECRET_TOKEN: z.string().optional().transform(v => v || undefined),
  ...
  // PostgreSQL
  DATABASE_URL: z.string().min(1, 'DATABASE_URL es requerido (e.g. postgresql://user:pass@localhost:5432/evva)'),
  ...
  GOOGLE_REDIRECT_URI: z.string().optional().transform(v => v || undefined),

  // Brave Search (opcional en fase 1)
  BRAVE_SEARCH_API_KEY: z.string().optional().transform(v => v || undefined),
});
